...
var configSchema = exports_external.object({
  ...
  proxmoxApiUrl: exports_external.string()
});
var toBase64 = (s) => {
...
var onHttpTrigger = (runtime2, payload) => {
  ...
  runtime2.log(`[Workflow 1] Received capture from device: ${deviceAddress}`);
  runtime2.log(`[Workflow 1] Hash: ${hash}`);
  runtime2.log(`[Workflow 1] Filename: ${filename}`);
  const imageHashBytes32 = hexToBytes32(hash);
  const capturedAtUnix = BigInt(Math.floor(new Date(timestamp).getTime() / 1000));
  const reportData = encodeAbiParameters(parseAbiParameters("bytes32 imageHash, bytes signature, address deviceAddress, string filename, uint256 capturedAt"), [
    imageHashBytes32,
    signature,
    deviceAddress,
    filename,
  ...
  runtime2.log(`[Workflow 1] Encoded ABI data: ${reportData}`);
  ...
  runtime2.log(`[Workflow 1] Pending tx: ${txHash}`);
  runtime2.log(`[Workflow 1] Etherscan: https://sepolia.etherscan.io/tx/${txHash}`);
  ...
  const postResp = httpClient.sendRequest(runtime2, (sendRequester, _config) => {
    const resp = sendRequester.sendRequest({
      url: `${runtime2.config.proxmoxApiUrl}/cre-pending`,
      method: "POST",
  ...
  runtime2.log(`[Workflow 1] Notified Proxmox, status: ${postResp.statusCode}`);
