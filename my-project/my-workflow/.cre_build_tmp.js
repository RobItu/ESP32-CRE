...
var onHttpTrigger = (runtime2, payload) => {
  ...
  runtime2.log(`[Workflow 2] Confirming capture: ${hash}`);
  runtime2.log(`[Workflow 2] Device: ${deviceAddress}`);
  const imageHashBytes32 = hexToBytes32(hash);
  const reportData = encodeAbiParameters(parseAbiParameters("bytes32 imageHash"), [imageHashBytes32]);
  runtime2.log(`[Workflow 2] Encoded ABI data: ${reportData}`);
  ...
  runtime2.log(`[Workflow 2] Confirmed tx: ${txHash}`);
  runtime2.log(`[Workflow 2] Etherscan: https://sepolia.etherscan.io/tx/${txHash}`);
  ...
  const postResp = httpClient.sendRequest(runtime2, (sendRequester, _config) => {
    const resp = sendRequester.sendRequest({
      url: `${runtime2.config.proxmoxApiUrl}/cre-callback`,
      method: "POST",
  ...
  runtime2.log(`[Workflow 2] Notified Proxmox, status: ${postResp.statusCode}`);
